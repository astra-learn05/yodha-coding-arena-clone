...
export const autoLoginToShaurya = async (user_id: string): Promise<string> => {
  try {
    const response = await fetch('https://tafvjwurzgpugcfidbfv.supabase.co/functions/v1/shaurya-auto-login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
